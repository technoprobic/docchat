package com.personal.healthy1.remote
...
class OkHttpHttpClient(builder: OkHttpHttpClientBuilder) : HttpClient {
    ...
    private fun handleSyncResponse(response: Response): SuccessfulHttpResponse {
        ...
        if (!response.isSuccessful) {
            Log.d("MYTAG", "handleSyncResponse not successful, http code: : ${response.code}")
            Log.d("MYTAG", "handleSyncResponse not successful, error: : ${responseBodyString ?: "no response"}")
